...
{
    public class ProcessingScenarioPWA500W_NRD : ProcessingScenario
    {
        public ProcessingScenarioPWA500W_NRD()
        {
            ...
            foreach (EN_SCENARIO scenario in Enum.GetValues(typeof(EN_SCENARIO)))
            {
                ...
                switch (scenario)
                {
                    ...
                    case EN_SCENARIO.SCENARIO_REQ_LOT_INFO_CORE_3:
                        {
                            MakeScenario(scenario, new ScenarioReqLotInfo(
                                ...
                                CollectionEventList[EN_EVENT_LIST.REQ_LOT_INFO_SUPPLY_PORT.ToString()].Id,
                                CollectionEventList[EN_EVENT_LIST.REQ_LOT_INFO_SUPPLY_PORT.ToString()].VariableIds,
                                14, 3, true
                                ));
                        }
                        break;
                    case EN_SCENARIO.SCENARIO_REQ_LOT_INFO_EMPTY_TAPE:
                        {
                            MakeScenario(scenario, new ScenarioReqLotInfo(
                                ObjectNames.LOTINFO.ToString(),
                                CollectionEventList[EN_EVENT_LIST.REQ_LOT_INFO_SORT_PORT.ToString()].Id,
                                CollectionEventList[EN_EVENT_LIST.REQ_LOT_INFO_SORT_PORT.ToString()].VariableIds,
                                14, 3, true
                                ));
                        }
                        break;
                    ...
                    case EN_SCENARIO.SCENARIO_RFID_READ_CORE_3:
                        {
                            MakeScenario(scenario, new SendingEventScenario(scenario.ToString(),
                                CollectionEventList[EN_EVENT_LIST.RFID_READ_SUPPLY_PORT.ToString()].Id,
                                CollectionEventList[EN_EVENT_LIST.RFID_READ_SUPPLY_PORT.ToString()].VariableIds,
                                true));
                        }
                        break;
                    case EN_SCENARIO.SCENARIO_RFID_READ_EMPTY_TAPE:
                        {
                            MakeScenario(scenario, new SendingEventScenario(scenario.ToString(),
                                CollectionEventList[EN_EVENT_LIST.RFID_READ_SORT_PORT.ToString()].Id,
                                CollectionEventList[EN_EVENT_LIST.RFID_READ_SORT_PORT.ToString()].VariableIds,
                                true));
                        }
                        break;
                    ...
                    case EN_SCENARIO.SCENARIO_REQ_SLOT_INFO_CORE_3:
                        {
                            MakeScenario(scenario, new ScenarioProceedWithCarrier(
                                CollectionEventList[EN_EVENT_LIST.REQ_SLOT_INFO_CORE_PORT.ToString()].Id,
                                CollectionEventList[EN_EVENT_LIST.REQ_SLOT_INFO_CORE_PORT.ToString()].VariableIds,
                                3,
                                17,
                                true));
                        }
                    ...
                    case EN_SCENARIO.SCENARIO_REQ_SLOT_INFO_EMPTY_TAPE:
                        {
                            MakeScenario(scenario, new ScenarioProceedWithCarrier(
                                CollectionEventList[EN_EVENT_LIST.REQ_SLOT_INFO_SORT_PORT.ToString()].Id,
                                CollectionEventList[EN_EVENT_LIST.REQ_SLOT_INFO_SORT_PORT.ToString()].VariableIds,
                                3,
                                17,
                                true));
                        }
                    ...
                    case EN_SCENARIO.SCENARIO_SLOT_WAFER_MAPPING_CORE_3:
                        {
                            MakeScenario(scenario, new SendingEventScenario(scenario.ToString(),
                                CollectionEventList[EN_EVENT_LIST.SLOT_WAFER_MAPPING_CORE_PORT.ToString()].Id,
                                CollectionEventList[EN_EVENT_LIST.SLOT_WAFER_MAPPING_CORE_PORT.ToString()].VariableIds,
                                true));
                        }
                        break;
                    case EN_SCENARIO.SCENARIO_SLOT_WAFER_MAPPING_EMPTY_TAPE:
                        {
                            MakeScenario(scenario, new SendingEventScenario(scenario.ToString(),
                                CollectionEventList[EN_EVENT_LIST.SLOT_WAFER_MAPPING_SORT_PORT.ToString()].Id,
                                CollectionEventList[EN_EVENT_LIST.SLOT_WAFER_MAPPING_SORT_PORT.ToString()].VariableIds,
                                true));
                        }
                        break;
                    ...
                    case EN_SCENARIO.SCENARIO_REQ_LOT_ID_MERGE_AND_CHANGE_BIN_1:
                        {
                            MakeScenario(scenario, new ScenarioReqLotMergeAndChange(
                                ...
                                CollectionEventList[EN_EVENT_LIST.REQ_LOT_MERGE_SORT.ToString()].Id,
                                CollectionEventList[EN_EVENT_LIST.REQ_LOT_MERGE_SORT.ToString()].VariableIds,
                                CollectionEventList[EN_EVENT_LIST.REQ_LOT_ID_CHANGE_SORT.ToString()].Id,
                                CollectionEventList[EN_EVENT_LIST.REQ_LOT_ID_CHANGE_SORT.ToString()].VariableIds));
                        }
                        break;
                    ...
                    // TODO : 빈소터와의 운영상 차이점 -> W는 RCMD가 없다. -> 작업 필요
                    ...
                    // TODO : 빈소터와의 운영상 차이점 -> W는 터미네이트되는 시나리오기 때문에 이벤트는 같지만, 발생 조건이 무조건 전량이다.
                    // 전량아닌 경우도 운영하려면 full split first, full split, trackout, slotmapping 등 이벤트 추가가 필요하다.
                ...
                case MessagesToReceive.RequestStartDetaching:
                    {
                        ...
                        // TODO : 빈소터와의 운영상 차이점 -> W는 파트정보가 다른 것을 인터락 걸어달라고 한 적이 없다.
                ...
                case MessagesToReceive.RequestSplitCoreChip:
                    {
                        ...
                        // 2026.03.10. jhlim [DEL] W도 전량 소팅이 아닌 경우도 추가되었다..
                        // TODO : 빈소터와의 운영상 차이점 -> W는 무조건 전량 소팅이다.. -> 추후 BIN처럼 나눠달라고 할듯..
                        // 현재는 Full Split 이 없기 때문에 false로 하드코딩
                        //isSplittedFully = false;
                        // 2026.03.10. jhlim [END]
                        return ExecuteScenarioToSplitChip(nameOfEq, isFirstSorting, isSplittedFully, coreSubstrateName, ringId, SubstrateType.Bin1, splitQty, binCode, userId);
                        #endregion
                    }
        ...
        //    if (false == receivedMessageFromPM.TryGetValue(EESKeys.KeySubstrateName, out string substrateName) ||
        //        receivedMessageFromPM.TryGetValue(EESKeys.KeyRingId, out string ringId))
        //    {
        ...
        //    if (false == GetCoreSubstrateByAttributes(substrateName, ringId, true, ref substrate))
        //    {
        //        WriteLog("Does not have substrate at process module");
        //        return false;
        //    }
        ...
        //    if (false == isCore)
        //    {
        //        coreLotId = substrate.GetAttribute(PWA500WSubstrateAttributes.CoreLotId);
        //        corePartId = substrate.GetAttribute(PWA500WSubstrateAttributes.CorePartId);
        //    }
        //    else
        //    {
        //        coreLotId = substrate.LotId;
        //        corePartId = substrate.GetAttribute(PWA500WSubstrateAttributes.PartId);
        //    }
        ...
        //    if (isCore)
        //    {
        //        resultParam[EESKeys.KeyParamESDSensor04] = _variablesToUpdate[StatusVariableIdForESDAtStage];
        //    }
