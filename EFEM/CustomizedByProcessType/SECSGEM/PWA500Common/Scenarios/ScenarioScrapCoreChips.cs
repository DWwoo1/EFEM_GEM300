...
{
    public class ScenarioScrapCoreChipsParamValues : ScenarioParamValues
    {
        public ScenarioScrapCoreChipsParamValues(Queue<List<string>> values)
        {
            VariableValues = new Queue<List<string>>(values);
        }

        public Queue<List<string>> VariableValues { get; }
    }

    public class ScenarioScrapCoreChips : AutoScenarioBase
    {
        #region <Constructor>
        public ScenarioScrapCoreChips(string name, long eventId,
        ...
        ScenarioScrapCoreChipsParamValues _paramValues = null;
        ...
        {
            _paramValues = paramValues as ScenarioScrapCoreChipsParamValues;
        }
        ...
        public override EN_SCENARIO_RESULT ExecuteScenario()
        {
            switch (_seqNum)
            {
                ...
                case (int)EN_SCENARIO_SEQ.SEND_MESSAGE:
                    {
                        if (_paramValues.VariableValues == null)
                            return ReturnScenarioResult(EN_SCENARIO_RESULT.ERROR);

                        if (_paramValues.VariableValues.Count == 0)
                        {
                            return ReturnScenarioResult(EN_SCENARIO_RESULT.COMPLETED);
                        }

                        var variables = _paramValues.VariableValues.Dequeue();
                        _gemHandler.SendEvent(_eventId, _statusVariableIds, variables.ToArray(), UseCheckSecondaryAck);
