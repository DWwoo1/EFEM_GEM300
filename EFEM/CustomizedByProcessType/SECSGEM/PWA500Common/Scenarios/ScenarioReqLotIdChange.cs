...
{
    public class ScenarioReqLotIdChangeParamValues : ScenarioParamValues
    {
        public ScenarioReqLotIdChangeParamValues(List<string> values) : base(values)
        {

        }
    }

    public class ScenarioReqLotIdChange : ScenarioSendEventThenHandlingSecsMessage
    {
        #region <Constructors>
        public ScenarioReqLotIdChange(string name, long eventId, List<long> variables,
        ...
        {
            _paramValue = paramValues as ScenarioReqLotIdChangeParamValues;
        }
        protected override bool UpdateReceivedSecsMessage(List<SemiObject> listOfReceive)
        {
            ...
            if (_paramValue != null)
            {
                // 2024.10.14. jhlim [DEL] 아이디를 부여받는 시나리오므로, 비교할 수 없어 폐기
