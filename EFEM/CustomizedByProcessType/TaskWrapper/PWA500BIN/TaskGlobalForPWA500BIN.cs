...
namespace FrameOfSystem3.Task
{
    class TaskGlobalForPWA500BIN : TaskGlobal
    {
        ...
        #region <Fields>
        private FunctionsForPWA500BIN_TP _functionsForPWA500 = null;
        ...
        protected override void ExecuteOnAlways()
        {
            if (_functionsForPWA500.HasScenarioError)
            {
                ...
                string subContent;
                if (string.IsNullOrEmpty(_functionsForPWA500.ScenarioErrorDescription))
                {
                    subContent = _functionsForPWA500.FailedScenarioTypes.ToString();
                }
                else
                {
                    subContent = string.Format("Name : {0}, Description : {1}", _functionsForPWA500.FailedScenarioTypes.ToString(), _functionsForPWA500.ScenarioErrorDescription);
                }
                string[] arAlarmSubInfo = { GetTaskName(), subContent };
