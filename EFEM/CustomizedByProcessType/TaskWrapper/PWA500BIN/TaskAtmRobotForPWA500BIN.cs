...
namespace FrameOfSystem3.Task
{
    class TaskAtmRobotForPWA500BIN : TaskAtmRobot
    {
                            ...
                            case ScenarioRecipeDownload:
                                {
                                    ...
                                    if (false == UpdateScenarioParam(scenario, scenarioParam))
                                    {
                                        _commandResult.CommandResult = CommandResult.Error;
                                ...
                                case ScenarioBinWaferIdAssign:
                                    {
                                        ...
                                        if (false == UpdateScenarioParam(ScenarioBinWaferIdAssign, scenarioParam))
                                        {
                                            _commandResult.CommandResult = CommandResult.Error;
                                            _commandResult.Description = "Failed to update for scenario param";
                                        }
        ...
        private CheckingCarrierCodeToUnload FindWellknownProtInfoBySubstrateType(Substrate substrate, SubstrateType subType, ref int portId, ref int slot, ref string description)
        {
            //CheckingCarrierCodeToUnload result;
                ...
                case SubstrateType.Bin3:
                    {
                        ...
                        for (int i = 1; i <= capacity; ++i)
                        {
                            if (i == 1 && (loadingMode == LoadPortLoadingMode.Cassette || loadingMode == LoadPortLoadingMode.ClosedCassette))
                                continue;
        ...
        private bool IsSourceSlotReservated(int portId, bool isCassette, out int targetSlot)
        {
            int capacity = _carrierServer.GetCapacity(portId);

            // LP
            var atLP = _substrateManager.GetSubstratesAtLoadPort(portId);

            var processModuleName = _processGroup.GetProcessModuleName(ProcessModuleIndex);
            List<Substrate> atPM = new List<Substrate>();
            var needCheckPM = _substrateManager.GetSubstratesAtProcessModule(processModuleName, ref atPM);

            Dictionary<RobotArmTypes, Substrate> atRb = new Dictionary<RobotArmTypes, Substrate>();
            var needCheckRobot = _substrateManager.GetSubstratesAtRobotAll(RobotName, ref atRb);
            ...
            for (int i = 1; i <= capacity; ++i)
            {
                ...
                // 1. LP
                // 슬롯에 자재가 있으면 -> 점유중, 없으면 공정모듈(Case:2) or 로봇(Case:3) 체크 필요. 순회할 필요가 없다.
                if (atLP.ContainsKey(i))
                    continue;

                //foreach (var item in atLP)
                //{
                //    if (item.Value == null)
                //        continue;

                //    if (item.Value.SourcePortId.Equals(portId) && item.Value.SourceSlot.Equals(i))
                //    {
                //        return true;
                //    }
                //}
                ...
                if (needCheckPM)
                {
                    foreach (var item in atPM)
                    {
                        if (item.SourcePortId.Equals(portId) && item.SourceSlot.Equals(i))
                        {
                            return true;
                        }
                    }
        ...
        private int FindUnknownSlotInfoByPortId(int portId, Substrate substrate, SubstrateType subType)
        {
            ...
            switch (subType)
            {
                ...
                case SubstrateType.Empty:
                    {
                        bool isCassetteMode = (_loadPortManager.GetCarrierLoadingType(lpIndex) == LoadPortLoadingMode.Cassette || _loadPortManager.GetCarrierLoadingType(lpIndex) == LoadPortLoadingMode.ClosedCassette);
                        if (false == IsSourceSlotReservated(portId, isCassetteMode, out int slot))
                        {
                            return slot;
                        }
                        //for (int i = 0; i < capacity; ++i)
                        //{
                        //    if (isCassetteMode && i == 0)
                        //        continue;

                        //    if (_substrateManager.HasSubstrateAtLoadPort(portId, i))
                        //        continue;

                        //    if (false == _substrateManager.IsSourceSlotReservated(portId, i))
                        //    {
                        //        return i;
                        //    }
                        //}
                ...
                case 3:
                    {
                        ...
                        else
                        {
                            ...
                            subType = substrate.GetAttribute(PWA500MaterialHandling.SubstrateType);
                            ...
                            ringId = substrate.GetAttribute(PWA500MaterialHandling.RingId);
                        }
                        ...
                        _substrateManager.SetAttributeByKey(key, PWA500MaterialHandling.SubstrateType, convertedType.ToString());
                        ...
                        var checkingResult = FindWellknownProtInfoBySubstrateType(substrate, convertedType, ref targetPortId, ref targetSlot, ref description);
                        //if (hasCarrier)
                        ...
                        if (checkingResult == CheckingCarrierCodeToUnload.DoorIsNotOpened)
                        {
                            return ReturnSkipped(MethodName);
                        }
        ...
        private bool HasDifferentPartInfo(string targetPartId, string substrateName, string substrateTypeToString, string parentPartId, string corePartId, ProcessingStates processingStatus, ref string description)
        {
            if (false == Enum.TryParse(substrateTypeToString, out SubstrateType substrateType))
                return false;

            string partIdToCompare;
            if (substrateType.Equals(SubstrateType.Core))
            {
                partIdToCompare = parentPartId;
            }
            else
            {
                if (processingStatus.Equals(ProcessingStates.NeedsProcessing))
                    return false;

                partIdToCompare = corePartId;
                if (string.IsNullOrEmpty(partIdToCompare))
                {
                    return false;
                    //description = "Does not have core part id in bin wafer";
                    //return true;
                }
            }

            if (false == targetPartId.Equals(partIdToCompare))
            {
                description = string.Format("Core PartId:{0}, Bin RingId:{1}, Bin PartId : {2}", targetPartId, substrateName, partIdToCompare);
                return true;
            }
        ...
        private bool IsValidPartIdsForSubstrates(ref Substrate substrate, ref string description)
        {
            ...
            string partIdToTrackIn = _carrierServer.GetAttribute(portId, PWA500CarrierAttributes.KeyPartId);
            ...
            if (_substrateManager.GetSubstratesAtProcessModule(processModuleName, ref _substratesAtProcessModule))
            {
                ...
                if (_substratesAtProcessModule.Count > 0)
                {
                    ...
                    {
                        substrateTypeToString = item.GetAttribute(PWA500SubstrateAttributes.SubstrateType);
                        processingStatus = item.ProcessingStatus;
                        parentPartId = item.GetAttribute(PWA500SubstrateAttributes.PartId);
                        corePartId = item.GetAttribute(PWA500SubstrateAttributes.CorePartId);
                                        ...
                                        case CarrierAccessStates.InAccessed:
                                            {
                                                ...
                                                if (substrates.Count > 0)
                                                {
                                                    ...
                                                    {
                                                        substrateTypeToString = substrateFirst.GetAttribute(PWA500SubstrateAttributes.SubstrateType);
                                                        processingStatus = substrateFirst.ProcessingStatus;
                                                        parentPartId = substrateFirst.GetAttribute(PWA500SubstrateAttributes.PartId);
                                                        corePartId = substrateFirst.GetAttribute(PWA500SubstrateAttributes.CorePartId);
                        ...
                        case SubstrateType.Bin3:
                            {
                                ...
                                {
                                    substrateTypeToString = item.Value.GetAttribute(PWA500SubstrateAttributes.SubstrateType);
                                    processingStatus = item.Value.ProcessingStatus;
                                    parentPartId = item.Value.GetAttribute(PWA500SubstrateAttributes.PartId);
                                    corePartId = item.Value.GetAttribute(PWA500SubstrateAttributes.CorePartId);
        ...
        public bool IsLastSubstrateAtLoadPortBeforePick(int portId, string key)
        {
